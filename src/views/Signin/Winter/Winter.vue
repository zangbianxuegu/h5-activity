...
// 活动数据
const activityData = computed(() => activityStore.eventData.activity_sign_in_2)
...
function getActivityData(): void {
  ...
  getPlayerMissionData({ event: 'activity_sign_in_2' })
    .then((res) => {
      const activityData: Event = res.data.event_data?.activity_sign_in_2[0]
      ...
      // 更新缓存活动数据
      activityStore.updateEventData('activity_sign_in_2', activityData)
    })
    .catch((error) => {
